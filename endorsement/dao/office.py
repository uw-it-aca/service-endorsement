...
from endorsement.exceptions import UnrecognizedGroupID, UnrecognizedUWNetid
...
def is_office_permitted(netid):
    ...
    try:
        subscription_codes = distinct(o365.subscription_codes + o365_student.subscription_codes)
        return active_subscriptions_for_netid(netid, subscription_codes)
    except Exception as ex:
        logger.error(f"is_office_permitted: {netid}: {ex}")
...
def validate_office_access(name):
    ...
    try:
        ...
        _uwregid, display_name, _email, is_person = get_endorsee_data(name)
...
def validate_group(name, display_name):
    ...
    try:
        ...
        logger.error(
            f"validate_office_access get_group {name}: {ex}")
        raise
